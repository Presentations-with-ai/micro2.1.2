...
const USER_NAME_BADGE_SUBTITLE = 'Доступ открыт';
...
const QUIZ_SECTION_CONFIGS = {
  ...
  marketing: {
    ...
    description: 'Второй раздел внутри Micro 2 Quiz',
    placeholderTitle: 'Раздел маркетинга готовится',
    placeholderText: 'Тесты по маркетингу будут добавлены в этот раздел позже.'
  }
};
...
function getSelectedTheme() {
  return document.getElementById('theme-select')?.value || 'micro_tests.json';
}

function getMainQuestionBankFile() {
  return 'micro_tests.json';
}
...
function initBanUi() {
  ...
  if (!document.getElementById('ban-overlay')) {
    ...
    banOverlay.innerHTML = `
      <div class="agreement-panel">
        <div class="agreement-badge">Премиум доступ</div>
        <h2 class="agreement-title">Введите ключ для активации доступа</h2>
        <p class="agreement-lead" data-role="activation-lead">Ключ создаётся по вашему Telegram ID и может открывать один раздел или оба сразу. Срок действия ключа — 30 дней.</p>
        ...
        <p class="premium-note" data-role="premium-access-note">После успешной активации доступ остаётся навсегда на этом устройстве.</p>
        <div class="agreement-actions">
          <button id="premium-activation-submit" class="main">Активировать</button>
...
function applyPremiumAccessOverlayContent(accessStatus = {}) {
  ...
  if (leadNode) {
    leadNode.textContent = normalizedUserId
      ? 'Доступ к сайту открыт для всех пользователей.'
      : 'Доступ к сайту открыт для всех пользователей.';
  }

  if (noteNode) {
    noteNode.textContent = normalizedUserId
      ? 'После активации доступ останется на этом устройстве. Тип ключа определяет, какие разделы будут открыты.'
      : 'Telegram ID пока не определён. Нажмите «Обновить ID» после открытия через Telegram.';
    noteNode.className = 'premium-note';
  }
...
function getThemeLabel(fileName) {
  const map = {
    '1-50.json': 'Вопросы 1-50',
    '51-100.json': 'Вопросы 51-100',
    '101-150.json': 'Вопросы 101-150',
    '151-200.json': 'Вопросы 151-200',
    '201-250.json': 'Вопросы 201-250',
    '251-301.json': 'Вопросы 251-301',
    'micro_tests.json': 'Все вопросы (Микс)',
    [FAVORITES_THEME_FILE]: 'Избранные вопросы'
  };
...
function loadStudyTests() {
  ...
  fetch('micro_tests.json')
    .then((response) => {
      if (!response.ok) throw new Error('Не удалось загрузить micro_tests.json');
...
function renderStudyList() {
  ...
  if (!studyState.items.length) {
    ...
    container.innerHTML = `
      <div class="history-empty">В файле micro_tests.json пока нет тестов.</div>
    `;
    return;
  }
...
<div class="card">
    ...
    <div class="row">
        <select id="theme-select">
            <option value="1-50.json">Вопросы 1-50</option>
            <option value="51-100.json">Вопросы 51-100</option>
            <option value="101-150.json">Вопросы 101-150</option>
            <option value="151-200.json">Вопросы 151-200</option>
            <option value="201-250.json">Вопросы 201-250</option>
            <option value="251-301.json">Вопросы 251-301</option>
            <option value="micro_tests.json" selected>Все вопросы (Микс)</option>
